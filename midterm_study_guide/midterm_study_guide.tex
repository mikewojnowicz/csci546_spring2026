...
\begin{abstract}
This midterm study guide is a \textbf{work in progress}.  In particular: (1)  Additional problems relevant to diffusion processes will be added soon. (2) The reference material is a work in progress.
\end{abstract}
...
\item (Gautam Iyer, \textit{\href{https://www.math.cmu.edu/~gautam/sj/teaching/2021-22/944-scalc-finance1/pdfs/ch3-int.pdf}{\underline{\blue{Stochastic Integration. (Lecture notes.)}}}}) \textbf{Total Variation of Brownian Motion.} Let $W$ be Brownian motion, and let $\Delta W_{k,n}$ denote the increment 
	...
	over the $k$-th bin of a partition $P = \set{0 = t_0 < t_1 < \hdots < t_n = t}$ of the interval $[0,t]$.    Let $T_n = \sum_{k=1}^{n} |\Delta W_{k,n}|$.  Compute $\lim_{n \to \infty} \E[T_n]$.  
	...
	\[ \Delta W_{k,n} = W \bigg( \frac{kt}{n} \bigg) -  W \bigg( \frac{(k-1)t}{n} \bigg)\] 
	over the $k$-th bin of a partition $P = \set{0 = t_0 < t_1 < \hdots < t_n = t}$ of the interval $[0,t]$.  Let $Q_n = \sum_{k=1}^{n} (\Delta W_{k,n})^2$.  Compute $\lim_{n \to \infty} \E[Q_n]$.
\end{enumerate}
...
\item A \textbf{standard Brownian motion} $\set{B(t): t \geq 0}$ is a stochastic process having
\begin{itemize}
...
%TOADD (Diffusion): Def of stationary increments, independent increments.
%TOADD (Markov Chain): Ehrenfest model. Recurrent, transient.
\end{enumerate}

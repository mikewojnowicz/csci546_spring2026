...
\small 
\only<1-4>{
\begin{center} \includegraphics[width=.8\textwidth]{images/forward_process_bishop.png} 
\end{center}	
}
\only<5>{
\begin{center} \includegraphics[width=.6\textwidth]{images/forward_process_bishop.png} 
\end{center}	
}
...
\only<5->{\colorbox{yellow!30}{\textbf{Poll.}} This clarifies interpretation. What happens to \Eqref{eqn:diffusion_kernel} as $t$ increases?
}
...
\begin{center} \includegraphics[width=.8\textwidth]{images/elbo} 
\end{center}
...
\end{align} %
 \hfill  \scripttext{for $t=1,\hdots,T$ and where we define $\+z_0=\+x$. \qquad}

The \textbf{diffusion kernel} is obtained by marginalizing over $\+z_1,\hdots, \+z_{t-1}$:
%
\begin{align}
 q(\+z_t \mid \+x) &= \N\bigg( \sqrt{\alpha_t}  \+x \, , \, (1-\alpha_t) \+I\bigg), \qquad \text{where} \quad \alpha_t \defeq \prod_{s=1}^t (1-\beta_s)  \label{eqn:diffusion_kernel} 
\end{align}
\vfill 
...
\begin{itemize}
	...
	\item \textit{Hint:} Rewrite \Eqref{eqn:forward_transition_kernel_2} as 
	\[ \+z_t = \sqrt{1-\beta_t}  \+z_{t-1} + \sqrt{\beta_t} \+\epsilon_t, \qquad  \+\epsilon_t \sim \N(\+0,\+I).   \]
	Then use the fact that the sum of two independent Gaussian random variables is itself a Gaussian with additive means and covariances.
\end{itemize} 
...
\begin{figure}
 \includegraphics[width=.8\textwidth]{images/group_exercise_soln_1} 	
\end{figure}
...
\begin{figure}
 \includegraphics[width=.8\textwidth]{images/group_exercise_soln_2} 	
\end{figure}

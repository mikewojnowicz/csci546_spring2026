...
\begin{figure} \includegraphics[width=.6\textwidth]{images/predicting_the_noise_results} 
\end{figure}
...
\only<1>{
\colorbox{yellow!30}{\textbf{Poll.}}  Our objective is tto minimize $\KL{q(\+z_{t-1} \mid \+z_t, \+x)}{p(\+z_{t-1} \mid \+z_t, \+w)}$.  What is this term doing?  
}
...
%
\begin{align*}
\labeledredmathbox{approximate denoising}{p(\+z_{t-1} \mid \+z_t, \+w)} &= \N \bigg(\+\mu_t(\+z_t, \+w) \; , \; \sigma^2_t \+I \bigg).
\end{align*}
...
\begin{align*}
\frac{1}{2\beta_t} \; \norm{\labeledgreenmathbox{true denoising mean}{\+m_t(\+z_t, \+x)}  -  \labeledredmathbox{approximate denoising mean}{\+\mu_t(\+z_t, \+w)}}^2 \; + \; \texttt{const}
\end{align*}
...
When the approximate denoised mean $\+\mu_t(\+z_t,\+w)$ takes the same form but with $\widehat{\+\epsilon}_t(\+z_t, \+w)$,  this leads to the \qq{predicting the noise} formulation. %(Bishop pp. 591).
\pause 

...
\begin{myredbox}[title=``Local balance'': a stronger condition than reversibility]
\begin{align*}
\explaintermbrace{total flow from $j$ to $j$}{\pi_j P_{ji}} = \explaintermbrace{total flow from $i$ to $j$}{\pi_i P_{ij}} \\
\end{align*}
\end{myredbox}
\pause 
\vfill 
...
\begin{center}
\includegraphics[width=.6\textwidth]{images/poisson_process}	
\end{center}
\vfill
...
\only<4>{ Hence, the expected value is $E[N(t)]=\lambda t$.
}
\only<5>{
...
\begin{mygreenbox}[title=\text{The only continuous distribution with the ``lack of memory'' property.}]
The \textbf{exponential distribution} is the only continuous distribution with the `lack of memory` property \[P(X>s+t \mid X>s) = P(X>t).\]
\end{mygreenbox}
\end{frame}

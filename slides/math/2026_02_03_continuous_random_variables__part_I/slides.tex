...
%{TODO} Why exponential tends to be a good model for time-to-event? See Sec 4.7 of Ross.
\end{frame}
...
%\only<3-4>{\colorbox{red!30}{\textbf{Interpretation.}} If we think of $X$ as waiting time for the next snowfall, \Eqref{eqn:memoryless} says:
%
%\vspace{.2cm}
%
%\begin{quote}
%\textit{If we're still waiting for snowfall at time $t$, the distribution of the remaining
%amount of time that we must wait is the same as the original waiting distribution.}
%\end{quote}
%}
%
%
%
%\only<4>{That is, it is as if Mother Nature not ``remember'' that we've already been waiting for time $t$.}
...
\begin{quote}
\textit{If we're still waiting for snowfall at time $t$, the distribution of the remaining
amount of time that we must wait is the same when we started waiting.}
\end{quote}
}

\only<4>{That is, it is as if Mother Nature does not ``remember'' that we've already been waiting for time $t$.}
...
\only<3-4>{
    \centering
    \includegraphics[width=\textwidth]{images/skiing}
}
\end{column}
\end{columns}

...
\item \textit{Path 1}: Play with \bluelink{https://e-dorigatti.github.io/math/deep\%20learning/2023/06/25/diffusion.html}{Emilio's code}.  Some things you might do:
	...
	\item Adjust codebase per ideas from Bishop.  
	\end{itemize} \pause 
\item \textit{Path 2}: Code up Bishop's algorithms from scratch, using Emilio's code as a guide, and apply it to some simple dataset.  \pause Note: this is harder because Bishop's algorithms are based on ``predicting the noise'' (Sec 20.2.4).
\end{enumerate}
\pause 
